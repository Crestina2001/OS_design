...
本项目以 《Oranges’一个操作系统的实现》作为基本框架，使用相关的核心代码。主要开发了内核程序。完成了 3 个系统级应用：类Shell控制台、进程管理和文件管理。同时，还实现了 5 个用户级应用，包括日历、gcd计算器和 3个小游戏。以类Shell方式与用户进行交互，通过命令行实现了简单的图形界面。

分工如下：

`[hidden name]`：组长，负责环境搭建、框架梳理、代码整合与项目管理工作。

`[hidden name]`：组员，负责文件系统。

`[hidden name]`：组员，负责进程管理。

`[hidden name]`：组员，负责编写小游戏推箱子。

`[hidden name]`：组员，负责编写用户级应用日历和小游戏2048。
...
本应用实现了文本与文件夹文件的创建，编辑，删除与搜索等功能。

.txt文件的创建：
![](assets/2.1_fileCreation.png)

在创建的时候，能自动检测在该层是否存在文件名重复，若存在，则无法创建。

文件夹的创建：
![](assets/2.1_folderCreation.png)

在创建的时候，同样能自动检测在该层是否存在文件重名，若存在，则无法创建。

文件列表的查看：
![](assets/2.1_listElement.png)

查看某一层的所有文件，上图显示了刚才所创建的文件

文件的搜索：
![](assets/2.1_fileSearch.png)
在全局使用文件名进行搜索，若找到则显示该文件的绝对路径。若在全局范围存在多个同名文件，则会将所有同名文件一并搜出，并按照路径由浅至深进行排列。

文本文件的编辑：
![](assets/2.1_txtEditer.png)
如图所示，能够对文本进行更新，查看，并将更新暂存在内存之中。

打开文件夹/返回上一级：
![](assets/2.1_cd.png)

删除文件：
![](assets/2.1_delete.png)
对文件进行删除，并且递归地删除该文件的所有子文件。
