...
//search by name
int searchByName(char *path, int *name, int i, int offset)
{
	int len = 0;
	while (blocks[i].fileName[len] != 0)
	{
		len++;
	}
	//printf("%d\n",len);
	if (strcmp(name, blocks[i].fileName) == 0)
	{
	...
	if (blocks[i].childrenNumber == 0)
		return -1;
	strcpy(path + offset, blocks[i].fileName);
	offset += len;
	path[offset] = '/';
	offset++;
	int res = -1;
	for (int j = 0; j < blocks[i].childrenNumber; ++j)
	{
		if (searchByName(path, name, blocks[i].children[j], offset) == 1)
		{
			res = 1;
		}
	}
...
void runFileManage(int fd_stdin)
{
	...
	while (1)
	{
		...
		else if (rdbuf[0] == 'm' && rdbuf[1] == 'k' && rdbuf[2] == 'f' && rdbuf[3] == 'd')
		{
			if (rdbuf[4] != ' ')
			{
				printf("You should add the dirname, like \"mkfd XXX\".\n");
				printf("Please input [help] to know more.\n");
				continue;
			}
		...
		else if (rdbuf[0] == 'd' && rdbuf[1] == 'e' && rdbuf[2] == 'l' && rdbuf[3] == 'e' && rdbuf[4] == 't' && rdbuf[5] == 'e')
		{
			if (rdbuf[6] != ' ')
			{
				printf("You should add the filename or dirname, like \"delete XXX\".\n");
				printf("Please input [help] to know more.\n");
				continue;
			}
			for (int i = 0; i < MAX_FILE_NAME_LENGTH && i < r - 3; i++)
		...
		else if (rdbuf[0] == 'f' && rdbuf[1] == 'i' && rdbuf[2] == 'n' && rdbuf[3] == 'd')
		{
			if (rdbuf[4] != ' ')
			{
				printf("You should add the filename, like \"find XXX\".\n");
				printf("Please input [help] to know more.\n");
				continue;
			}
			for (int i = 0; i < MAX_FILE_NAME_LENGTH && i < r - 3; i++)
			...
			for (int i = 0; i < 1000; ++i)
				_path[i] = 0;
			int is_found = searchByName(_path, _name, 0, 0);
			//printf("%s\n",_path);
			if (is_found == -1)
				printf("Not Found!\n");
		}
...
int CreateFIle(char *fileName, int fileType)
{
	...
	else
	{
		for (int i = 0; i < blocks[currentFileID].childrenNumber; i++)
		{
			...
			{
				if (blocks[blocks[currentFileID].children[i]].fileType == 1)
				{
...
void ShowMessage()
{
	...
	printf("      #     $ mkfd [dirname]    |   create a new folder                  #\n");
	printf("      #     $ find [filename]   |   search by name globally              #\n");
	...
	printf("      #     $ sv                |   save to the disk                     #\n");
	printf("      #     $ quit              |   quit the interface                   #\n");
	printf("      #                      Powered by unknownOS                         #\n");
...
void CommandList()
{
	...
	printf("      #                             $ gcd [a,b]                          #\n");
	printf("      #                                   --- calculate gcd of a and b   #\n");
...
void howMain(char *option)
{
	...
	else if (!strcmp(option, "play"))
	{
		...
		printf("      #               ppp               -KFC                             #\n");
		printf("      #              ppp                    --- Play KFC order game      #\n");
		printf("      #             ppp                                                  #\n");
	...
	else if (!strcmp(option, "gcd"))
	{
		clear();
		printf("      ====================================================================\n");
		printf("      #                                                                  #\n");
		printf("      #    gcdgcdgcgcd                                                   #\n");
		printf("      #   gcdgcdgcgcdgcd                                                 #\n");
		printf("      #  gcd          gcdg                                               #\n");
		printf("      #                 gcd                                              #\n");
		printf("      #                 gcd                                              #\n");
		printf("      #                gcd             <COMMAND --- gcd>                 #\n");
		printf("      #               gcd                Input \"a,b\"                   #\n");
		printf("      #              gcd            For example, $ gcd 8,12              #\n");
		printf("      #             gcd          returns the result which is 4           #\n");
		printf("      #            gcd                                                   #\n");
		printf("      #           gcd                                                    #\n");
		printf("      #                                                                  #\n");
		printf("      #           gcd                                                    #\n");
	...
	else if (!strcmp(option, "process"))
	{
		...
		printf("      #   procprocprocpr                                                 #\n");
		printf("      #  pro          proc                                               #\n");
		printf("      #                 pro                Enter $ process               #\n");
		printf("      #                 pro                  to test our                 #\n");
		printf("      #                pro            process management system          #\n");
		printf("      #               pro                                                #\n");
		printf("      #              pro                 When you are entered            #\n");
		printf("      #             pro             run \"$ help\" to see further info   #\n");
		printf("      #            pro                                                   #\n");
		printf("      #           pro                                                    #\n");
		printf("      #                                                                  #\n");
		printf("      #           pro                                                    #\n");
	...
	else if (!strcmp(option, "file"))
	{
		...
		printf("      #  file          file                                              #\n");
		printf("      #                 file                 Enter $ file                #\n");
		printf("      #                 file            to test our file system          #\n");
		printf("      #                file                                              #\n");
		printf("      #               file                when entered you can           #\n");
		printf("      #              file          run \"$ help\" to see further info    #\n");
		printf("      #             file                                                 #\n");
		printf("      #            file                                                  #\n");
		printf("      #           file                                                   #\n");
		printf("      #                                                                  #\n");
...
void gcdMain(char *exp)
{
	int a, b;
	char a_str[3] = "\0", b_str[3] = "\0";
	if (!strcmp(exp, "NULL"))
	{
		printf("Sorry please add 2 integer in range 1-999 in format \"a,b\"");
		printf("For example : $ gcd 1,2");
	}
	else
	{
		if (exp[0] < '0' || exp[0] > '9')
...
void runKFC(int fd_stdin)
{
	...
	while (1)
	{
		...
		int ans_money = 0;
		const int spe_len = 4;
		const int list_len = 14;
		int disnum[spe_len][list_len];
		int menuchoice[list_len];
		...
		for (int temp_i = 0; temp_i < spe_len; temp_i++)
		{
			printf("%s=", special[temp].name);
			for (unsigned int i = 0; i < 3; i++)
			{
				disnum[temp_i][special[temp].sign[i] - 'A']++;//计数
			}
			for (int i = 0; i < list_len; i++)
			{
				if (disnum[temp_i][i] > 0)
				{
					printf("%s", list[i].name);
		...
		printf("[Rules]:\n");
		printf("ABG=Zinger Burger+Beef Burger+Sandwich\n");
		printf("Input exit and you will exit!");
		printf("\n");
		printf("=============================================================\n");
		printf("Please Order: ");
		...
		{
			if (!((readbuffer[i] >= 'A' && readbuffer[i] <= 'N') || (readbuffer[i] >= 'a' && readbuffer[i] <= 'n')))
				wrong_flag = 1;
			else if (readbuffer[i] >= 'a' && readbuffer[i] <= 'n')
				readbuffer[i] -= 32;
		}
		if (wrong_flag != 0)
		{
			...
			while (1)
			{
				printf("Please Input again to start again or Input exit to exit!\n");
				char tempstr[128];
				int end1 = read(fd_stdin, tempstr, 70);
				tempstr[end1] = 0;
				if (strcmp(tempstr, "again") == 0)
				{
					clear();
					break;
				}
				else if (strcmp(tempstr, "exit") == 0)
				{
					clear();
					return;
				}
		...
		else
		{
			...
			for (int i = 0; i < list_len; i++)
			{
				if (menuchoice[i] > 0)
				{
					printf("%s", list[i].name);
					if (menuchoice[i] > 1)
						printf("*%d", menuchoice[i]);
					int temp_sum = 0;
					for (int k = i; k < list_len; k++)
					{
						temp_sum += menuchoice[k];
					}
					if (temp_sum > menuchoice[i])
						printf("+");
					else
						printf("\n");
				}
			...
			for (int i = 0; i < spe_len; i++)
			{
				...
				while (1)
				{
					...
					{
						if (disnum[i][k] > menuchoice[k])//不满足折扣
						{
							dis_flag = 1;
						}
					}
					...
					else
					{
						...
						for (int j = 0; j < list_len; j++)
						{
							menuchoice[j] -= disnum[i][j];
						}
						ans_money += special[i].price;
					}
				}
			}
			for (int k = 0; k < list_len; k++)
			{
				ans_money += menuchoice[k] * list[k].price;
			}
			printf("Total money=");
			printf("%d", ans_money);
			printf("\n");
			...
			while (1)
			{
				printf("Please Input again to start again or Input exit to exit!\n");
				char tempstr[128];
				int end2 = read(fd_stdin, tempstr, 70);
				tempstr[end2] = 0;
				if (strcmp(tempstr, "again") == 0)
				{
					clear();
